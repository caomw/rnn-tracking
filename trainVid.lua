...
This file trains a multi-layer RNN to predict bounding box locations of pedestrians in a video sequence.

Code is based on implementation in 
https://github.com/karpathy/char-rnn.git
...
-- parse input params
opt = cmd:parse(arg)
torch.manualSeed(opt.seed)	--reset Torch random number generator seed
...
local loader = MinibatchLoader.create(opt.data_dir, opt.batch_size, opt.seq_length, split_sizes, opt.input_size)
--local vocab_size = loader.vocab_size  -- the number of distinct characters
-- vocab_size is likely to be equivalent to our input size, so replaced with opt.input_size - Shawn Rigdon 6/22/15
--local vocab = loader.vocab_mapping
print('input size: ' .. opt.input_size)
...
if string.len(opt.init_from) > 0 then	-- check to see if there is a checkpoint to load model parameters from
    print('loading an LSTM from checkpoint ' .. opt.init_from)
    local checkpoint = torch.load(opt.init_from)
    protos = checkpoint.protos

	--[[ I assume I won't need the vocab comparison
...
-- make a bunch of clones after flattening, as that reallocates memory			-- I don't understand cloning the prototypes does exactly
clones = {}
...
function eval_split(split_index, max_batches)
    ...
    for i = 1,n do -- iterate over batches in the split
        ...
        for t=1,opt.seq_length do
			...
			local lst = clones.rnn[t]:forward{x[{{}, {}, t}]:t(), unpack(rnn_state[t-1])}
            rnn_state[t] = {}
            for i=1,#init_state do table.insert(rnn_state[t], lst[i]) end
            prediction = lst[#lst] 
            --loss = loss + clones.criterion[t]:forward(prediction, y[{{}, t}])
			loss = loss + clones.criterion[t]:forward(prediction, y[{{}, {}, t}]:t())
        end
...
function feval(x)
    ...
    local predictions = {}           -- softmax outputs		--CHANGED TO LINEAR OUTPUT
    local loss = 0
    for t=1,opt.seq_length do
        ...
        loss = loss + clones.criterion[t]:forward(predictions[t], y[{{}, {}, t}]:t())
    end
