...
cmd:option('-data_dir','data/test','data containing rnn inputs to be tracked')
cmd:option('-out_dir','output','directory to write predictions.txt')
-- optional parameters
cmd:option('-seed',123,'random number generator\'s seed')
--cmd:option('-sample',1,' 0 to use max at each timestep, 1 to sample at each timestep')
--cmd:option('-primetext',"",'used as a prompt to "seed" the state of the LSTM using a given sequence, before we sample.')
--cmd:option('-length',2000,'number of characters to sample')
cmd:option('-frames',1,'number of frames ahead to make prediction')
...
function forwardPass(netInput)
	local lst = protos.rnn:forward{ netInput, unpack(current_state) }
	--print(lst[5])
	current_state = {}
...
local input_txtFile = path.join(opt.data_dir, 'input.txt')
local input_tensorFile = path.join(opt.data_dir, 'data.t7')
local output_txtFile = path.join(opt.out_dir, 'predictions.txt')
-- use MinibatchLoader.text_to_tensor to create tensor from tracking input file
local seq_length = 0
for line in io.lines(input_txtFile) do seq_length = seq_length+1 end
MinibatchLoader.text_to_tensor(input_txtFile, input_tensorFile, seq_length, checkpoint.opt.input_size)
